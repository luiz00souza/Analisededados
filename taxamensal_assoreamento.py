...
def meses_entre(data_inicio, data_fim):
    return max(1, (data_fim.year - data_inicio.year)*12 + (data_fim.month - data_inicio.month))

# ---------------------- FUNÇÃO MEDIA NORMALIZADA ----------------------
def media_direta_normalizada(arquivos):
    dfs = []
    for arq in arquivos:
        ...
        n_meses = meses_entre(data_inicio, data_fim)
        df["Z_mensal"] = df["Z"] / n_meses
        dfs.append(df[["X","Y","Z_mensal"]])
    
    df_all = pd.concat(dfs, ignore_index=True)
    df_final = df_all.groupby(["X","Y"], as_index=False)["Z_mensal"].mean()
    df_final.rename(columns={"Z_mensal":"Z_final"}, inplace=True)
    return df_final
...
# XYZ média direta normalizada
df_media = media_direta_normalizada(uploaded_files)
xyz_buffer = io.StringIO()
df_media.to_csv(xyz_buffer, sep="\t", index=False)
st.download_button("📊 Baixar Média Direta Normalizada (XYZ)", xyz_buffer.getvalue(), "Z_media_direta_normalizada.xyz", "text/plain")

st.balloons()
st.success("🎉 Análise concluída com sucesso! Explore e baixe os resultados 🌱")
